...
def main():
    ...
    filename_with_path = os.path.join(compilation_temp,'results'+name_id, filename)
    with open(filename_with_path, 'rb') as f:
        output = pickle.load(f) 
        
    output_dir = os.path.join(outputs, "results"+name_id)
    ...
    #        c += 1
        ...
        ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0))  
        ax.get_figure().savefig(os.path.join(output_dir,'sim_reward_plot_k_'+str(seed_set)+name_id+'.eps'))
        ax.get_figure().savefig(os.path.join(output_dir,'sim_reward_plot_k_'+str(seed_set)+name_id+'.jpg'))
    ...
    fig.savefig(os.path.join(output_dir,'reward_legend.png'), dpi="figure", bbox_inches=bbox)
    
    #TODO asking about this overwriting previous results
    if regret_plots == 'yes':
        for seed_set in seed_sets:   
            chosen_for_regret = []
            for colname in colnames:
    ...
    fig.canvas.draw()
    bbox  = legend.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    fig.savefig(os.path.join(output_dir,'regret_legend.png'), dpi="figure", bbox_inches=bbox)
